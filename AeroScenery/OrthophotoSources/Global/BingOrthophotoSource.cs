...
namespace AeroScenery.OrthophotoSources
{
    ...
    {
        // Taken verbatim from what bing.com/maps' own aerial style uses
        // (window.v10ControlConfig.mapStyleDefinition.styles[1].tilePrimer.urlBase), so it stays on a
        // host Microsoft actually serves rather than ecn., their deprecated edge network, over http.
        //
        // The imagery is the same either way: g=42 and g=15615 were hash-identical over 12 test tiles,
        // so g does not select the imagery vintage here despite looking like a version. n=z makes a
        // no-coverage tile come back as zero bytes instead of a 1033-byte placeholder; either way it
        // carries the X-VE-Tile-Info: no-tile header that DownloadFile keys off.
        public static string DefaultUrlTemplate = "https://t.ssl.ak.tiles.virtualearth.net/tiles/a{0}.jpeg?g=15615&n=z&prx=1";
