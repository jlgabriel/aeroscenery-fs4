...
namespace AeroScenery.FileManagement
{
    public class FileManager
    {
        ...
        public async Task DeleteGridSquareFilesAsync(string gridSquareDirectory, bool mapTiles, bool stitchedImages, bool ttcFiles)
        {
            var task = Task.Run(() =>
            {
                ...
                foreach (DirectoryInfo directoryInfo in di.GetDirectories())
                {
                    ...
                    foreach (DirectoryInfo dir in innerDirectoryInfo.GetDirectories())
                    {
                        ...
                        // A -raw folder is left over from versions that ran IPACS GeoConvert. It
                        // belongs to the same step as the ttc files, so it goes with them.
                        if (dir.Name.Contains("-raw") && ttcFiles)
                        {
